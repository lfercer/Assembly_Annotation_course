...
table(rep_table$fam)
# How many elements are there in each Superfamily? 47?? 47 columns?
...
ggplot(rep_table.m, aes(fill = fam, x = distance, weight = value / 1000000)) +
  ...
  xlab("Distance") +
  ylab("Sequence (Mbp)") +
  theme(axis.text.x = element_text(angle = 90, vjust = 1, size = 14
...
ggplot(rep_table.m, aes(fill = fam, x = age/10^6, weight = value / 1000000)) +
  geom_bar() +
  cowplot::theme_cowplot() +
  scale_fill_brewer(palette = "Paired") +
  xlab("Age (Myr)") +
